...
- a44c43d: Semantic search only embeds files with a body and within the new per-vault Embedding Scope setting (Markdown / Documents / Everything). Vectors for files that fall out of scope are dropped on the next embedding pass, and non-markdown documents re-embed when their content changes.
- a44c43d: Tags are now opt-in: an inline `#tag` is a dimmed candidate until promoted from its pill, the tag pane's Candidates section, or by accepting it from `#` autocomplete. Frontmatter tags are promoted automatically. Suggested links only consider promoted tags.

### Patch Changes

- a44c43d: Tags inside inline code, indented code, and math blocks are no longer indexed, and the editor no longer treats purely numeric `#123` as a tag, so the tag pane and editor pills agree.
...
- 8bae587: Fix semantic similarity scoring and how those scores are presented.
...
- d880cd8: Stop indexing hidden dotfiles and MCP config files, so chat sources no longer cite `.gitignore` or `mcp.json`. Existing indexes prune these rows on the next vault sync; no re-index is required.
- a060430: Stop a settings save from silently abandoning most of your global settings
...
- b14b3a8: HTML authoring integration for agents and humans.
...
- 64774c9: Preserve a note's indexed child rows and cross-path columns on every save
...
- c1cfbc3: Improve daily note workflows with stable Markdown language-server recovery, persistent embed folding, safer block-link authoring, configurable AI context budgets, folder-aware chat and omnibar navigation, reversible omnibar sorting, and focus-preserving editor overlays.
