...
export const COMMAND_TO_ACTION_ID: Record<CommandId, string> = {
  ...
  toggle_bases_panel: ACTION_IDS.bases_toggle_panel,
  ...
  zoom_in: ACTION_IDS.editor_zoom_in,
  zoom_out: ACTION_IDS.editor_zoom_out,
  zoom_reset: ACTION_IDS.editor_zoom_reset,
  ...
  iwe_extract_all: ACTION_IDS.iwe_extract_all,
  iwe_inline_section: ACTION_IDS.iwe_inline_section,
  iwe_inline_quote: ACTION_IDS.iwe_inline_quote,
  iwe_list_to_sections: ACTION_IDS.iwe_list_to_sections,
  iwe_section_to_list: ACTION_IDS.iwe_section_to_list,
  iwe_sort_list: ACTION_IDS.iwe_sort_list,
  iwe_create_link: ACTION_IDS.iwe_create_link,
  delete_note: ACTION_IDS.note_request_delete,
  duplicate_note: ACTION_IDS.note_duplicate,
  copy_as_markdown: ACTION_IDS.note_copy_markdown,
...
export function register_omnibar_actions(input: ActionRegistrationInput) {
  ...
  registry.register({
    id: ACTION_IDS.omnibar_toggle_kind_filter,
    label: "Toggle Omnibar Kind Filter",
    execute: (filter: unknown) => {
      const f = filter as OmnibarKindFilter;
      const current = stores.ui.omnibar.kind_filters;
      const next = current.includes(f)
        ? current.filter((x) => x !== f)
        : [...current, f];
      set_omnibar_state(input, { kind_filters: next, selected_index: 0 });
      apply_omnibar_view(input);
    },
  });

  registry.register({
    id: ACTION_IDS.omnibar_set_sort_mode,
    label: "Set Omnibar Sort Mode",
    execute: (mode: unknown) => {
      set_omnibar_state(input, {
        sort_mode: mode as OmnibarSortMode,
        selected_index: 0,
      });
      apply_omnibar_view(input);
    },
