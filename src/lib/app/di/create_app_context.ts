...
import { FolderService, register_recents_actions } from "$lib/features/folder";
...
export function create_app_context(input: {
  ...
  register_chat_actions({
    ...
    documents: assistant_documents,
    assistant_kernel,
    permissions: input.ports.assistant_permissions,
    assistant_proposals: stores.assistant_proposals,
  });
  ...
  register_recents_actions({
    registry: action_registry,
    bases_port: input.ports.bases,
    recents_store: stores.recents,
