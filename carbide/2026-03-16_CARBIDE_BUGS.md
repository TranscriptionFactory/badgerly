...
**Resolution:** `Cmd+Shift+F` is already bound to `omnibar_open_all_vaults` (searches across all vaults). `Cmd+O` opens omnibar for current vault. `Cmd+P` opens command palette. All three registered in `default_hotkeys.ts`. Discoverability issue only — no code change needed.
...
**Resolution:** Implemented via `handleTextInput` plugin (same pattern as emoji plugin). Supports 6 arrow substitutions: `-->` → `→`, `<--` → `←`, `<->` → `↔`, `==>` → `⇒`, `<==` → `⇐`, `<=>` → `⇔`. Skips code blocks and math blocks. Em dash and ellipsis intentionally excluded — conflict with Markdown syntax. See `carbide/sprints/2026-03-16_typographic_substitution.md` for details.
...
### 13. Tags UI surface — IMPLEMENTED
...
5\. Find & Replace — table-stakes editor feature
...
~~10\. Configurable note naming (#10)~~ DONE
~~11\. Date link auto-suggest (#11)~~ DONE

**Backlog (P3 — new features):**
~~12\. Tags UI (#13)~~ DONE, Calendar (#14), Anchor links (#18)
13\. Full-vault graph (#17) — per PRIORITIES.md sequencing
14\. Theme expansion (#15), Plugin maturation (#16)
~~15\. Code highlighting upgrade (#8)~~ DONE, Draggable sections (#19)
