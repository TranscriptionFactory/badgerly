import type { VaultSettingsPort } from "$lib/features/vault";
import type { VaultId } from "$lib/shared/types/ids";

export function create_test_vault_settings_adapter(): VaultSettingsPort {
  const storage = new Map<string, Map<string, unknown>>();
  const local_storage = new Map<string, Map<string, unknown>>();

  function get_vault_storage(
    target: Map<string, Map<string, unknown>>,
    vault_id: VaultId,
  ): Map<string, unknown> {
    let vault_storage = target.get(vault_id);
    if (!vault_storage) {
      vault_storage = new Map();
      target.set(vault_id, vault_storage);
    }
    return vault_storage;
  }

  return {
    get_vault_setting<T>(vault_id: VaultId, key: string): Promise<T | null> {
      const vault_storage = get_vault_storage(storage, vault_id);
      const value = vault_storage.get(key);
      return Promise.resolve(value !== undefined ? (value as T) : null);
    },

    set_vault_setting(
      vault_id: VaultId,
      key: string,
      value: unknown,
    ): Promise<void> {
      const vault_storage = get_vault_storage(storage, vault_id);
      vault_storage.set(key, value);
      return Promise.resolve();
    },
    get_local_setting<T>(vault_id: VaultId, key: string): Promise<T | null> {
      const vault_storage = get_vault_storage(local_storage, vault_id);
      const value = vault_storage.get(key);
      return Promise.resolve(value !== undefined ? (value as T) : null);
    },
    set_local_setting(
      vault_id: VaultId,
      key: string,
      value: unknown,
    ): Promise<void> {
      const vault_storage = get_vault_storage(local_storage, vault_id);
      vault_storage.set(key, value);
      return Promise.resolve();
    },
  };
}
